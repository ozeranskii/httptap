...
class TestMaskSensitiveValue:
    ...
    def test_mask_long_value_shows_first_and_last_chars(self) -> None:
        """Test masking long values shows first and last characters."""
        result = mask_sensitive_value("Bearer token123456", show_chars=4)
        assert result == "Bear****3456"

    def test_mask_short_value_returns_full_mask(self) -> None:
        """Test that short values are fully masked."""
        result = mask_sensitive_value("short", show_chars=4)
        assert result == "****"

    def test_mask_exact_length_returns_full_mask(self) -> None:
        """Test that values exactly 2*show_chars long are fully masked."""
        result = mask_sensitive_value("12345678", show_chars=4)
        assert result == "****"

    def test_mask_custom_show_chars(self) -> None:
        """Test masking with custom show_chars parameter."""
        result = mask_sensitive_value("secret123456", show_chars=2)
        assert result == "se****56"
...
class TestSanitizeHeaders:
    ...
    def test_sanitize_headers_masks_sensitive_values(
        ...
        headers = {header: value, "X-Trace": "trace"}
        sanitized = sanitize_headers(headers)
        assert sanitized[header] != value
        assert sanitized["X-Trace"] == "trace"
    ...
    def test_sanitize_headers_preserves_non_sensitive(self) -> None:
        """Test that non-sensitive headers are preserved."""
        headers = {
            "Content-Type": "application/json",
            "User-Agent": "httptap/1.0",
            "X-Custom": "value",
        }
        sanitized = sanitize_headers(headers)
    ...
    def test_sanitize_headers_api_key_variants(self) -> None:
...
class TestParseHttpDate:
    ...
    def test_parse_valid_http_date(self) -> None:
        """Test parsing valid RFC 7231 HTTP date."""
        date_str = "Mon, 22 Oct 2025 12:00:00 GMT"
    ...
    def test_parse_invalid_http_date_returns_none(self) -> None:
        """Test that invalid date format returns None."""
        result = parse_http_date("invalid date")
        assert result is None

    def test_parse_http_date_wrong_format_returns_none(self) -> None:
        """Test that wrong date format returns None."""
        result = parse_http_date("2025-10-22 12:00:00")
        assert result is None

    def test_parse_http_date_missing_gmt_returns_none(self) -> None:
        """Test that dates without GMT suffix return None."""
        result = parse_http_date("Mon, 22 Oct 2025 12:00:00 UTC")
        assert result is None
...
class TestParseCertificateDate:
    ...
    def test_parse_valid_certificate_date(self) -> None:
        """Test parsing valid certificate date format."""
        date_str = "Oct 22 12:00:00 2025 GMT"
    ...
    def test_parse_invalid_certificate_date_returns_none(self) -> None:
        """Test that invalid certificate date returns None."""
        result = parse_certificate_date("invalid date")
        assert result is None

    def test_parse_certificate_date_wrong_format_returns_none(self) -> None:
        """Test that wrong format returns None."""
        result = parse_certificate_date("2025-10-22 12:00:00")
        assert result is None
